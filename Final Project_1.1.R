...
ggplot(distance_country, aes(x=orig_destination_distance,y=count,colour=hotel_country))+ geom_line()+coord_flip()+
  scale_colour_gradientn(colours=rainbow(4))
...
#**********************************************************************************
#----------------------------------------------------------------------------------
clean_data = read.csv(file = "clean_data.csv")
View(clean_data)
clean_data2=clean_data[!(is.na(clean_data$orig_destination_distance)),]

#counting missing values
missing_values <- sapply(clean_data2, function(x) sum(is.na(x)))
missing_values
summary(clean_data2)
clean_data3 = clean_data2
clean_data3$target = clean_data2$hotel_cluster
?subset
clean_data3 = subset(clean_data3, select = -hotel_cluster)
...
target
ggplot(data=target,
       aes(y=count,x=reorder(target,count)))+
  xlab("target")+
  ylab("count")+
  geom_bar(stat = "identity",fill='#1E90FF')+
  geom_text(aes(label=count),hjust=0)+
...
set.seed(1)
train_id =sample(nrow(clean_data3), nrow(clean_data3)*0.7)
train_data = clean_data3[train_id, ]
test_data = clean_data3[-train_id, ]
#----------------------------------------------------------------------------------
#**********************************************************************************
#----------------------------------------------------------------------------------
